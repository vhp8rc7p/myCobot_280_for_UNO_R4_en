...
>
> The steps below install **M5Stack** board support, which is required only
> when the main control *is* an M5Stack. When the main control is an Arduino
> board (UNO, MEGA, MKR or UNO R4), the M5Stack board package and the M5Stack
> library are **not** used by the build — compiling
> `MyCobot280_Arduino/Mkr/AnglesControl` reports exactly one library in use,
> `MyCobotBasic`.
>
> For UNO R4 setup, see [6.1.1 Environment Construction](10.1-arduino_download.md).
