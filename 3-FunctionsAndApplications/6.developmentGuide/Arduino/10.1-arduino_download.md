...
> If you intend to use the **Transponder** example, consider
> [this fork](https://github.com/vhp8rc7p/MyCobotBasic) instead - it adds a
> `UNO_R4` examples folder with the two fixes below already applied. The
> official release needs both applied by hand before the transponder works on
> an R4.
...
> ### `Mkr/Transponder.ino` needs two fixes on the UNO R4
>
> **Fix 1 — makes it compile.**
>
> ```
...
>
> **Fix 2 — makes it work.** With only fix 1 applied it compiles and uploads,
> but `pymycobot` gets nothing back: `get_angles()` returns `-1` even though
> the arm is replying correctly.
>
> `AtomDataToClient()` detects a `FE FE` header before forwarding:
>
> ```cpp
> rx_byte = Serial1.read();
> if (rx_byte == HEADER && (temp = Serial1.read()) == HEADER) { ... }
> ```
>
> That second read is unguarded. On the UNO R4 the arm's bytes arrive one at a
> time, so it usually finds an empty buffer, returns `-1`, swallows the `0xFE`
> and drops the frame. `flag` is also a local, so it resets every call and a
> frame split across calls can never be forwarded. Replace the body with a
> plain forward:
>
> ```cpp
...
>     count = 0;
>     int guard = 0;
>     while (Serial1.available() > 0 && guard++ < 64) {
>         Serial.write((uint8_t)Serial1.read());
>     }
> }
> ```
>
> **To skip both**, use the pre-fixed examples at
> [vhp8rc7p/MyCobotBasic](https://github.com/vhp8rc7p/MyCobotBasic/tree/main/examples/MyCobot280/MyCobot280_Arduino/UNO_R4)
> - a fork with a `UNO_R4` folder where both fixes are already applied.
>
